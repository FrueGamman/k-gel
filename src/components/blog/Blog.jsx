...
export default function Blog() {
  ...
  useEffect(() => {
    const getBlog = async function () {
      try {
        const response = await api.get(`${backEndPoints.BLOG}`)
  ...
  return (
    <div>
      ...
      <div className="xl:px-20 px-8 py-20 2xl:mx-auto 2xl:container relative z-40">
        <CarouselProvider naturalSlideWidth={100} isIntrinsicHeight={true} totalSlides={blogs.length}>
          ...
          <Slider>
            {blogs.map((item, index) => {
              return (
                <Slide index={item.id}>
                  <div className="flex relative z-0" style={{ transform: 'translateX(0%)' }}>
                    <div className="mt-14 md:flex">
                      <div className="relative lg:w-1/2 sm:w-96 xl:h-96 h-80">
                        <a href={`${item.link}`} target="_blank">
                          <img src={`${apiUrl}${item.Image}`} alt="image of profile" className="w-full h-full flex-shrink-0 object-fit object-cover shadow-lg rounded"
                          /></a>
                      ...
                      <div className="md:w-1/3 lg:w-1/3 xl:ml-32 md:ml-20 md:mt-0 mt-4 flex flex-col justify-between">
                        <div>
                          <h1 className="text-2xl font-semibold xl:leading-loose text-gray-800">{item.title}</h1>
                          <p className="text-base font-medium leading-6 mt-4 text-gray-600">{item.description}</p>
                        </div>
                        <div className="md:mt-0 mt-8">
                          <p className="text-base font-medium leading-4 text-gray-800">{item.name}</p>
                          <p className="text-base leading-4 mt-2 mb-4 text-gray-600">{item.date}</p>
                        </div>
                      </div>
                    </div>
                  </div>
