...
export default function Pants() {
  ...
  const [dataPant, setData] = useState([]);
  
  useEffect(() => {
    const getSuit = async () => {
      setLoading(true);
      try {
        const response = await api.get(`${backEndPoints.FILTER_CLOTHERS_PANT}`);
  ...
  return (
    <div className="md:mx-24 py-12 max-x-4xl">
      <section className="grid md:grid-cols-5 grid-cols-2 sm:grid-cols-2  justify-center place-content-center md:mx-1">
        {loading ? (
